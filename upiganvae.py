...
def apply_validations_and_classify(df: pd.DataFrame) -> pd.DataFrame:
    ...
    def classify_row(r):
        ...
        cnt=len(flags)
        cls = "upi_fraud_type" if cnt>=2 else ("anomaly" if cnt==1 else "normal")
        return pd.Series({"triggered_validations":",".join(flags) if flags else None, "num_validations":cnt, "classification":cls})
...
# ---------------------------
# INJECTORS (guarantee anomalies appear)
# ---------------------------

def inject_V1_patterns(df: pd.DataFrame, num_targets=2, unique_senders=30, window_minutes=120):
    """
    Create num_targets new 'to_account' each with <90 days age and receive `unique_senders`
    transactions from different senders within a window_minutes window.
    """
    rows = []
    for _ in range(num_targets):
        target = gen_upi()
        # make target new (<90 days)
        to_age = random.randint(1, 89)
        # choose start time near now
        start = datetime.utcnow() - timedelta(hours=random.uniform(1,48))
        # create unique senders
...
def inject_V2_patterns(df: pd.DataFrame, num_accounts=5, incoming_per_account=6):
    """
    For num_accounts, create incoming transactions and immediate forwards within 5 minutes
    with forwarded amounts >=80% of received so they trigger V2.
    """
    rows = []
    for _ in range(num_accounts):
        acct = gen_upi()
        acct_age = random.randint(30, 1000)
        # create several incoming txs to acct
        for i in range(incoming_per_account):
            ...
            # forwarded tx within 5 minutes, >=80%
...
def inject_V3_patterns(df: pd.DataFrame, num_accounts=2, burst_count=60):
    """
    Create dormant accounts (historical low tx/day) and generate a burst of `burst_count`
    outgoing txs within one hour to trigger V3.
    """
    rows = []
    for _ in range(num_accounts):
        acct = gen_upi()
        acct_age = random.randint(100, 2000)
        # mark as historically dormant externally (account_hist can be used in detection)
        # generate burst within 1 hour
        start = datetime.utcnow() - timedelta(hours=random.uniform(1,48))
...
def run_pipeline(output_csv="upi_with_injected_anomalies.csv", synthetic_count=5000, seed=42):
    ...
    extras = pd.concat(
        [df for df in [v1_rows, v2_rows, v3_rows] if not df.empty],
        ignore_index=True
    ) if (not v1_rows.empty or not v2_rows.empty or not v3_rows.empty) else pd.DataFrame()
    ...
    # 4) Run validations & classification
...
# ---------------------------
# Glue: using our earlier CTGAN+VAE combined generator functions
# For convenience we re-use same generate_combined_upi defined above (it exists in this script),
# otherwise fallback to building base and sampling.
# ---------------------------
# (If the earlier generate_combined_upi function name isn't present in your context,
#  paste the generate_combined_upi implementation from prior script here.)
#
# For safety in this single-file, let's provide a minimal fallback generator:
def generate_combined_upi(base_df, ctgan_epochs=1, ctgan_n=1500, vae_epochs=1, vae_n=500):
    # Try CTGAN + VAE; if not available, fallback to sampling similar to prior code
    base_fixed = fix_upi_columns(base_df)
    try:
        ctgan_obj, train_cols = train_ctgan_fast_upi(base_fixed, epochs=ctgan_epochs)
        synth_ctgan = ctgan_generate_fast_upi(ctgan_obj, train_cols, n=ctgan_n, base_df=base_fixed)
    except Exception:
        logging.exception("CTGAN sampling failed, falling back.")
        synth_ctgan = base_fixed.sample(ctgan_n, replace=True).reset_index(drop=True).copy()
        synth_ctgan["timestamp"] = [next_timestamp() for _ in range(len(synth_ctgan))]
    # VAE
    try:
        vae_model, vae_enc, vae_dec, vae_scaler, vae_numeric_cols = build_train_vae_fast_upi(base_fixed, latent_dim=4, epochs=vae_epochs)
        if vae_enc is not None:
            df_num = vae_generate_fast_upi(vae_enc, vae_dec, vae_scaler, vae_numeric_cols, n=vae_n, latent_dim=4)
            rows=[]
            for i in range(len(df_num)):
                row = base_fixed.sample(1).iloc[0].to_dict()
                row.update(df_num.iloc[i].to_dict())
                row["tx_id"] = new_tx_id()
                rows.append(row)
            synth_vae = fix_upi_columns(pd.DataFrame(rows))
        else:
        ...
        logging.exception("VAE fallback failed.")
        synth_vae = pd.DataFrame()
